...
map = Image.new(
    "RGBA", (size[0] * 2 * multiplier, size[1] * 2 * multiplier), (0, 105, 148)
)
...
def get_box(polygon):
    x1, y1, x2, y2 = (
        min([x[0] for x in polygon]),
        min([y[1] for y in polygon]),
        max([x[0] for x in polygon]),
        max([y[1] for y in polygon]),
    )
...
for polygon, code in polygons:
    ...
    if w == 0 or h == 0:
        ...
        draw_mask.polygon(
            [(coordinates[0] - x, coordinates[1] - y) for coordinates in polygon],
            outline="black",
            fill="white",
        )
